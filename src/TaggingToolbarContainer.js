...
class TaggingToolbarContainer extends React.Component {
    ...
    handleChange = (tagGroup) => {
        return tagInCurrentGroup => {
            ...
            const tagsNotInCurrentGroup = removeTagIfInCurrentGroup(this.state.selectedTags);

            const getTagGroup = R.curry(this.findTagGroupByTagId)
            const isGroupActive = R.curry(this.isTagGroupActive(tagsNotInCurrentGroup))
    ...
    getOptionsForTagGroup = (tagGroup) => {
        ...
        const transformTagToOption = R.converge(
            addToValueKey,
            [R.prop("tag"), R.identity]
        );
